...
class HoughCircles(pipeline.ProcessObject):
    ...
    def generateData(self):
        ...
        print output.size
        print numpy.where(output)[0].size
        ...
        self.getOutput(0).setData(inpt)#bins[self.rad_idx]*100)
        self.getOutput(1).setData(output)
...
def test_circle_detection(args):
    ...
    display = Display(hCircles.getOutput(0), "1")
    
    display2 = Display(hCircles.getOutput(1), "2")
